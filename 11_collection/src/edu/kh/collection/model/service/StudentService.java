...
public class StudentService {
	...
	// List studentList = new ArrayList();   //빨간거 뜨면 import 해줘야함 
											 // 제네릭스 볼 때 주석처리함. 그래야 오류가 안떠요
	...
	public void ex() {
		...
		studentList.add(new Student() ); //0번 인덱스 //  빨간줄 import 해주기 경로 확인 잘하기!!
		//studentList.add(sc); //1번 인덱스   //제네릭스 작업할 때 주석처리
		//studentList.add("문자문자"); //2번 인덱스   //제네릭스 작업할 때 주석처리
		//studentList.add(new Object() ); //3번 인덱스   //제네릭스 작업할 때 주석처리
		...
		// 다운 캐스팅을 해야( get(int index) 메소드 반환형이 Object이기 때문에)
		// Student 기능을 사용할 수 있다.
		
		// 다운캐스팅 하려면 강제형변환 (student) 먼저 해주는거 맞는지 체크체크체크★★★ 
		...
		// ************************** 제네릭스(Generics) *********************** 
		// 표기법 : <타입> 
		// -> 컬렉션에 저장되는 객체 타입을 한가지로 제한. ex) student
		// 양쪽에다가 < > 표시하고 저장될 객체를 적어주면 됨. 예시 해당 클래스 30행 참고
				
		System.out.println( studentList.get(0).getName() );
		// 다운캐스팅 안해도 됨. 어차피 student니까
	}
	...
	public void selectAll() {
		...
		for(int i = 0; i<studentList.size(); i++) {
			...
			//studentList에서 1번째 인덱스 요소를 얻어와 출력
 		
 		*
 		*위 for문 향상된 for문 작업해야되서 주석처리함 
 		*/
	...
	public String updateStudent() throws InputMismatchException {
		...
		if(studentList.isEmpty() ) {
			...
			// 입력받은 index번째에 새롱누 학생 정보를 세팅 == 수정
	...
	public String removeStudent() throws InputMismatchException {
		...
		if(studentList.isEmpty() ) {
			...
			char ch = sc.next().toUpperCase().charAt(0);
						...
						// String.toUpperCase() : 문자열을 대문자로 고정 
	...
	public void searchName1() {
		...
		for( Student std : studentList ) {
			...
			if(input.equals(std.getName()) ) { // 이름이 일치하는 경우
				System.out.println(std); //std 뒤에 toString 찍지 않아도 값이 나옴...... 왜져.....?
				flag = false;
			}
	...
	public void searchName2() {
		...
		for( Student std : studentList ) {
			...
			// contains : 포함
			// boolean String.contains(문자열) : String에 문자열이 포함되어있으면 true를 반환
			if( std.getName().contains(input)) { // 포함되어있으면 true 아니면 false로 나옴
				System.out.println(std); 
				flag = false;
			}
