...
public class MemberService {
	...
	private Member[] memberArr = new Member[5];
	                                  // 멤버 줄쳐지면 글자에 대서 import 해줌
	
	// 현재 로그인한 회원의 정보를 저장할 변수 선언
	private Member loginMember = null;
	
	public MemberService() {  // 멤버서비스의 기본생성자
	...
	public String login() { //숙제숙제 
		...
		for(int i = 0; i < memberArr.length; i++) {  // for문 사용
			if(memberArr[i] != null) { 	// 회원 정보가 있을 경우  // if 사용
	...
	public int updateMember() {
		...
		// 2) 수정할 회원 정보 입력 받기 (이름,나이, 지역만 수정가능하도록)
		System.out.print("수정할 이름 입력 : ");
		String inputName = sc.next();
		
		System.out.print("수정할 나이 입력 : ");
		int inputAge = sc.nextInt();
		sc.nextLine();  // 입력버퍼 정리 인트다음 스트링 쓰는 경우 개행해주기 
		
		System.out.print("수정할 이름 지역 : ");
	...
	public void searchRegion() {
		...
		String inputRegion = sc.next(); // inputRegion 이라는 지역변수 만듬 
		...
		// 1) memberArr 배열의 모든 요소 순차 접근   (for문???
		for(int i=0; i < memberArr.length; i++) {
			// 2) memberArr[i] 요소가 null 경우 반복 종료 (if, break)
			if(memberArr[i] == null) break;
			
			// 3) memberArr[i] 요소에 저장된 지역(getResion())이  (if)
